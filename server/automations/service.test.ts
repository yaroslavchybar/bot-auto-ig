...
test('checkpoint bursts coalesce, stay off websocket, and survive a state-less terminal event', async () => {
  ...
  try {
    await runAutomation({ automationId: 'burst' }, () => proc)
    ...
    automationWorkers.delete('burst')
    globalThis.fetch = originalFetch
  }
})

test('worker payload uses the newly started automation rather than the old completed snapshot', async () => {
  const originalFetch = globalThis.fetch
  let payload = ''
  ...
  try {
    await runAutomation({ automationId: 'test' }, () => proc)
    assert.deepEqual(JSON.parse(payload).automation.nodeStates, {})
    assert.equal(JSON.parse(payload).automation.name, 'Fresh')
    proc.emit('close', 0)
    await tick()
  } finally { globalThis.fetch = originalFetch; automationWorkers.delete('test') }
})

test('automation remains owned until the terminal update finishes; late old events cannot alter a replacement', async () => {
  const originalFetch = globalThis.fetch
  ...
  automationWorkers.set('test', { process: proc, status: 'running', startedAt: 0 })
  try {
    wireProcessLifecycle(proc, 'test')
    proc.emit('close', 0)
    await tick()
    assert.equal(automationWorkers.get('test')?.process, proc)
    finish()
    await tick()
    assert.equal(automationWorkers.has('test'), false)
    const replacement = new EventEmitter() as ChildProcess
    automationWorkers.set('test', { process: replacement, status: 'running', startedAt: 1 })
    proc.emit('close', 0)
    await tick()
    assert.equal(calls, 1)
    assert.equal(automationWorkers.get('test')?.process, replacement)
  } finally { globalThis.fetch = originalFetch; automationWorkers.delete('test') }
})
