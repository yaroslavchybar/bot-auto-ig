...
test('parseLogLine handles prefixed event lines that contain the legacy sentinel text', () => {
    const parsed = parseLogLine('__EVENT__{"type":"task_progress","ts":"2026-03-13T12:00:00+00:00","detail":"checkpoint }__EVENT__ reached","profileName":"alice"}')
...
test('parseLogLine still accepts legacy wrapped event lines', () => {
    const parsed = parseLogLine('__EVENT__{"type":"profile_started","ts":"2026-03-13T12:00:00+00:00","profileName":"alice"}__EVENT__')

    assert.ok(parsed)
    assert.equal(parsed.eventType, 'profile_started')
    assert.equal(parsed.metadata?.profileName, 'alice')
...
test('parseLogLine handles structured events after stream and timestamp prefixes', () => {
    const parsed = parseLogLine('- [pid=12345][out] [2026-03-13T12:00:00+00:00] __EVENT__{"type":"profile_started","profileName":"alice"}')
...
test('parseLogLine treats array event payloads as plain log lines', () => {
    const parsed = parseLogLine('__EVENT__["profile_started",{"profileName":"alice"}]')

    assert.ok(parsed)
    assert.equal(parsed.eventType, undefined)
    assert.equal(parsed.metadata, undefined)
    assert.equal(parsed.message, '__EVENT__["profile_started",{"profileName":"alice"}]')
})
