...
export async function smoothScroll(page: Page, total: number): Promise<void> {
...
async function settleOnPost(page: Page): Promise<void> {
  const vp = page.viewportSize() ?? { width: 1280, height: 800 }
  const articles = page.locator('article')
  const n = Math.min(await articles.count().catch(() => 0), 6)
  let best = -1
  let bestFrac = -1
  for (let i = 0; i < n; i++) {
  ...
  await alignPostOnScreen(page, articles.nth(best))
}

// Settle the post to 90-100% on screen with wheel input only.
// Posts taller than the viewport align to the top edge instead.
export async function alignPostOnScreen(page: Page, target: Locator): Promise<void> {
  const vp = page.viewportSize() ?? { width: 1280, height: 800 }
  for (let i = 0; i < 4; i++) {
    ...
    await smoothScroll(page, Math.max(-800, Math.min(800, delta)))
...
export async function revealForClick(page: Page, el: Locator): Promise<boolean> {
  const vp = page.viewportSize() ?? { width: 1280, height: 800 }
  for (let i = 0; i < 3; i++) {
    ...
    await smoothScroll(page, Math.max(-600, Math.min(600, delta)))
...
export async function scrollModal(page: Page, dialog: Locator, ticks = 3): Promise<void> {
  try {
