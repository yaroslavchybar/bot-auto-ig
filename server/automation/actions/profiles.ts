...
import { random, sleep, type ActionLogger, type StopCheck } from './shared.js'
...
export async function openAuthorProfile(page: Page, target: Locator): Promise<boolean> {
  try {
    ...
    await revealForClick(page, link).catch(() => false)
...
async function pickDialogUser(page: Page): Promise<boolean> {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    await revealForClick(page, target).catch(() => false)
    await sleep(random(300, 700))
    await Promise.all([
      page.waitForURL(PROFILE_URL, { timeout: 8_000 }).catch(() => undefined),
      target.click({ timeout: 5_000 }).catch(() => undefined),
    ])
...
async function openLikersProfile(page: Page, target: Locator): Promise<boolean> {
  try {
    const likers = target
      .locator('span[role="button"]', { hasText: /^[\d\s.,KMB]+$/ })
      .first()
    if (!(await likers.isVisible().catch(() => false))) return false
    await likers.click({ timeout: 5_000 })
    return await pickDialogUser(page)
  } catch {
...
async function openCommentsProfile(page: Page, target: Locator): Promise<boolean> {
  try {
    const icon = target
    ...
    try {
      ...
      for (let i = 0; i < looks; i++) {
        await smoothScroll(page, random(250, 500))
...
export async function openVisitorProfile(page: Page, target: Locator): Promise<boolean> {
  if (await openLikersProfile(page, target)) return true
  return openCommentsProfile(page, target)
}

// Own profile via the sidebar avatar button. UI only, no direct navigation.
export async function openOwnProfile(page: Page, log: ActionLogger): Promise<boolean> {
  try {
    const own = page
      .locator('a[href^="/"]')
      .filter({ has: page.locator('img[alt$="profile picture"]') })
      .filter({ hasText: 'Profile' })
      .first()
    if (!(await own.isVisible().catch(() => false))) return false
    await own.click({ timeout: 5_000 })
    await page.waitForURL(PROFILE_URL, { timeout: 10_000 }).catch(() => undefined)
    await sleep(random(1200, 2500))
...
export async function wanderProfile(
  ...
  for (let i = 0; i < rounds && Date.now() < end && !shouldStop(); i++) {
    await smoothScroll(page, random(400, 800))
  ...
  for (const idx of hlOrder) {
    ...
    await highlight.click({ timeout: 5_000 }).catch(() => undefined)
    await sleep(random(2500, 5000))
    if (Math.random() < 0.5)
      await clickVisible(page, '[aria-label*="Next"], button:has-text("Next")').catch(
        () => undefined,
      )
  ...
  for (let i = 0; i < upGlides && Date.now() < end && !shouldStop(); i++) {
    await smoothScroll(page, -random(350, 650))
