...
import { AutomationEdge, AUTOMATION_EDGE_DEFAULTS } from './AutomationEdge'
  ...
  return (
    <ReactFlowProvider>
      <AutomationFlowEditorInner
        key={props.automation?._id ?? 'automation'}
        {...props}
      />
    </ReactFlowProvider>
  )
...
function AutomationFlowEditorInner({
  ...
  const state = useFlowEditorState(automation)
  ...
  return (
    <div className="bg-shell text-ink flex h-full w-full flex-col overflow-hidden p-2 font-sans text-xs md:p-3">
      ...
      <AutomationEditorProvider value={state.editorContextValue}>
    ...
    <div className="border-line-soft bg-panel/40 relative min-h-0 flex-1 overflow-hidden rounded-2xl border shadow-xs">
      ...
      >
        ...
        <ReactFlow
          ...
          minZoom={0.4}
          maxZoom={1.75}
          defaultEdgeOptions={AUTOMATION_EDGE_DEFAULTS}
        >
