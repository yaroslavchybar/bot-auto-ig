...
function LogEntries({ logs, loading }: { logs: LogEntry[]; loading: boolean }) {
  ...
  if (logs.length === 0) {
    return (
      <div className="text-subtle-copy italic">No activity recorded.</div>
    )
  }
    ...
    <div className="flex h-full flex-col gap-0">
      ...
      <div className="bg-shell text-copy min-h-0 flex-1 overflow-hidden font-mono text-[11px] leading-relaxed">
        <ScrollArea className="h-full">
          <div className="p-4">
            <LogEntries logs={logs} loading={loading} />
          </div>
        </ScrollArea>
      </div>
    </div>
