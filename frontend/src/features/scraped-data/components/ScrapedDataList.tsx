    ...
    <div className="bg-panel-strong border-line hover:border-line-strong rounded-2xl border p-4 shadow-xs transition-colors">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <p className="text-subtle-copy mt-1 truncate text-[11px]">{artifact.workflowName}</p>
        </div>
      ...
      <MobileCardStats artifact={artifact} />
    </div>
  )
}

function MobileCardStats({ artifact }: { artifact: WorkflowArtifact }) {
  return (
    <>
      <div className="text-muted-copy mt-4 flex flex-wrap items-center gap-2 text-xs">
        ...
        {artifact.sourceProfileName && (
          <span className="text-copy truncate">{artifact.sourceProfileName}</span>
        )}
      </div>
      <div className="border-line mt-4 flex items-center justify-between gap-3 border-t pt-3">
        <div className="min-w-0">
          ...
          <div className="text-copy mt-1 text-xs">{formatNumber(getArtifactRowCount(artifact))}</div>
        </div>
    ...
    <TableRow className="group border-line-soft h-14 border-b transition-colors hover:bg-panel-subtle">
      <TableCell className="pl-4 font-medium">
        <div className="flex flex-col gap-0.5">
          <span className="text-ink truncate">{artifact.name || 'Untitled artifact'}</span>
  ...
  return (
    <TableHeader>
      <TableRow className="border-line-soft border-b bg-transparent hover:bg-transparent">
        ...
        <TableHead className="text-muted-copy h-12 w-[100px] font-medium">Kind</TableHead>
        <TableHead className="text-muted-copy h-12 w-[160px] font-medium">Source</TableHead>
