    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <h3 className="text-ink mt-1 truncate text-base font-semibold">{list.name}</h3>
          <p className="text-subtle-copy mt-2 truncate font-mono text-[11px]">{list.id}</p>
        </div>
      ...
      <div className="border-line mt-4 border-t pt-3">
        <Button
          ...
          onClick={(event) => { event.stopPropagation(); onEdit(list) }}
        >
...
function ListDesktopRow({
  ...
  return (
    ...
    >
      ...
      <TableCell>
        <span className="text-subtle-copy truncate font-mono text-[10px]">{list.id}</span>
      </TableCell>
...
export function ListsList({ lists, loading, onEdit, onDelete }: ListsListProps) {
  ...
  if (lists.length === 0) {
    return (
      <div className="border-line-soft bg-panel-subtle flex flex-col items-center justify-center rounded-2xl border-2 border-dashed p-12 text-center">
        ...
        <p className="text-subtle-copy mt-1 text-sm">Create a new list to organize your profiles.</p>
      </div>
    )
  }
  ...
  if (isMobile) {
    return (
      <div className="space-y-3">
        {lists.map((list, idx) => (
          <ListMobileCard key={list.id} list={list} idx={idx} onEdit={onEdit} onDelete={onDelete} />
        ))}
      </div>
    )
  }
      ...
      <Table>
        <TableHeader>
          <TableRow className="border-line-soft border-b bg-transparent hover:bg-transparent">
            ...
            <TableHead className="text-muted-copy h-12 w-full font-medium">Name</TableHead>
            <TableHead className="text-muted-copy h-12 w-[180px] font-medium">List ID</TableHead>
            <TableHead className="text-muted-copy h-12 w-[140px] pr-4 text-right font-medium">Actions</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {lists.map((list, idx) => (
            <ListDesktopRow key={list.id} list={list} idx={idx} onEdit={onEdit} onDelete={onDelete} />
          ))}
        </TableBody>
      </Table>
    </div>
