...
const details = (lead: Lead) => [
  ...
  ['Imported', new Date(lead.createdAt).toLocaleDateString()],
]

export function LeadsList({ leads, loading, hasActiveFilter }: { leads: Lead[]; loading: boolean; hasActiveFilter: boolean }) {
  const mobile = useIsMobile()
  if (loading) return <p className="text-muted-copy p-12 text-center">Loading leads...</p>
  if (!leads.length) return <div className="border-line bg-panel-subtle rounded-2xl border p-12 text-center">
    ...
    <p>{hasActiveFilter ? 'No matching leads' : 'Import leads to start outreach.'}</p>
  </div>
  ...
  return <div className="bg-panel-subtle border-line overflow-hidden rounded-2xl border"><Table>
    <TableHeader><TableRow>{['Username', 'DM sent', 'Followed', 'Follow date', 'Sender', 'Imported'].map(label => <TableHead key={label}>{label}</TableHead>)}</TableRow></TableHeader>
    <TableBody>{leads.map(lead => <TableRow key={lead._id}><TableCell><Username lead={lead} /></TableCell>{details(lead).map(([label, value]) => <TableCell key={label} className="text-xs whitespace-nowrap">{value}</TableCell>)}</TableRow>)}</TableBody>
  </Table></div>
}
