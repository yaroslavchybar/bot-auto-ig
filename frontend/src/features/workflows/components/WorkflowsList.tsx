...
function WorkflowActionsMenu(props: WorkflowActionsMenuProps) {
  const [open, setOpen] = useState(false)

  const handleEditFlowClick = () => {
    setOpen(false)
    window.requestAnimationFrame(() => props.onEditFlow(props.workflow))
  }
  ...
  return (
    <div className="bg-panel-strong border-line hover:border-line-strong rounded-2xl border p-4 shadow-xs transition-colors">
      <MobileCardHeader
        ...
        menuProps={{ ...menuProps, workflow, isRunning, onEditSchedule }}
      />
      <MobileCardFooter
        workflow={workflow}
        isActive={isActive}
...
function MobileCardHeader({
  ...
  hasSchedule: boolean
  menuProps: WorkflowActionsMenuProps
}) {
  return (
    <div className="flex items-start justify-between gap-3">
      <div className="min-w-0 flex-1">
        ...
        {workflow.maxRunsPerDay && hasSchedule && (
          <div className="mt-3">
            <Badge
              variant="outline"
...
/* ── Schedule cell for desktop table ── */
  ...
  return (
    <TableHeader>
      <TableRow className="border-line-soft border-b hover:bg-transparent">
        ...
        <TableHead className="text-muted-copy w-[200px] font-medium">Schedule</TableHead>
...
export function WorkflowsList({
  workflows, loading, onToggleActive, onStopRun,
  onEdit, onEditFlow, onEditSchedule, onDuplicate, onExport, onDelete, onViewDetails,
}: WorkflowsListProps) {
  const isMobile = useIsMobile()

  if (loading) {
    return <div className="text-subtle-copy p-8 text-center">Loading workflows...</div>
  }
  if (workflows.length === 0) {
    return <div className="text-subtle-copy p-8 text-center">No workflows found. Create one to get started.</div>
  }

  const sharedMenuProps = { onStopRun, onViewDetails, onEditSchedule, onEditFlow, onEdit, onDuplicate, onExport, onDelete }
