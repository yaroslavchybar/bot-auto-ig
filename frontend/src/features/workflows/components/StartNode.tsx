...
function StartNodeComponent({ id, selected }: NodeProps<StartNodeData>) {
  return (
    <div
      className={`group bg-panel workflow-node relative flex w-[174px] flex-col overflow-visible rounded-[3px] border ${selected ? 'workflow-node-selected border-line-strong' : 'border-line'} `}
    >
      <NodeActionToolbar
        nodeId={id}
        selected={selected}
      ...
      <div className="relative flex h-full w-full">
        ...
        <div className="flex w-full flex-col pl-1">
          <div className="border-line-soft bg-panel-subtle flex items-center gap-2 border-b px-2 py-1.5 pr-24">
            <div className="bg-status-success-soft border-status-success-border flex h-4.5 w-4.5 shrink-0 items-center justify-center rounded-[2px] border">
              <Play
                className="text-status-success h-2.5 w-2.5"
                fill="none"
                strokeWidth={2}
              />
            </div>
          ...
          <div className="group/output border-line-soft text-subtle-copy flex items-center justify-between border-t bg-transparent px-2 py-1 font-mono text-[8px] tracking-[0.22em] uppercase">
