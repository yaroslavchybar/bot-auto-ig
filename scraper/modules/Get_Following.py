...
def get_userid(to_scrape_username, proxy=None):
    ...
    logger.warning(
        'following.get_userid unresolved target_username=%s proxy=%s error_code=%s error_message=%s',
        to_scrape_username,
...
def get_data_chunk(user_id, username, session_id, cursor=None, chunk_limit=200, max_pages=10, proxy=None):
    ...
    if result.outcome != 'success':
        logger.warning(
            'following.get_data_chunk non_success target_username=%s outcome=%s error_code=%s error_message=%s',
            username,
            result.outcome,
            result.error_code,
            result.error_message,
