...
from python.core.utils import apply_count_limit, create_status_callback
...
def run_scrolling(runner, page, mode: str) -> None:
    try:
        ...
        if mode == 'feed':
            log(f'Feed: {duration} min')
            scroll_feed(page, duration, config, should_stop=lambda: not runner.running)
            return
        log(f'Reels: {duration} min')
        scroll_reels(page, duration, config, should_stop=lambda: not runner.running)
    except Exception as exc:
        log(f'Scrolling error: {exc}')
...
def run_follow(runner, page, account, profile_data: Optional[Dict[str, Any]] = None) -> None:
    try:
        ...
        if not profile_id:
            log('Profile not found in DB.')
            return
        accounts = runner.accounts_client.get_accounts_for_profile(profile_id)
        usernames = _apply_session_limit(_usernames(accounts), runner.config.follow_count_range, 'Follow')
        if not usernames:
            log('No targets for follow.')
            return
        follow_usernames(
            ...
            on_success=_follow_success_callback(runner, _account_map(accounts)),
            on_skip=_follow_skip_callback(runner, _account_map(accounts)),
        )
    except Exception as exc:
        log(f'Follow error: {exc}')
...
def _follow_success_callback(runner, account_map: Dict[str, str]):
    return create_status_callback(
...
def _follow_skip_callback(runner, account_map: Dict[str, str]):
    return create_status_callback(
        runner.accounts_client,
        account_map,
        log,
...
def run_unfollow(runner, page, account, profile_data: Optional[Dict[str, Any]] = None) -> None:
    try:
        ...
        if not profile_id:
            log('No profile data for unfollow.')
            return
        accounts = runner.accounts_client.get_accounts_for_profile(profile_id, status='unsubscribed')
        usernames = _apply_session_limit(_usernames(accounts), runner.config.unfollow_count_range, 'Unfollow')
        if not usernames:
            log('No assigned accounts for unfollow.')
            return
        unfollow_usernames(
            profile_name=account.username,
            proxy_string=account.proxy or '',
            ...
            on_success=create_status_callback(
                runner.accounts_client,
...
def _apply_session_limit(usernames: List[str], count_range, label: str) -> List[str]:
    limited = apply_count_limit(usernames, count_range)
    if count_range:
        log(f'{label} session limit: {len(limited)}')
    return limited
