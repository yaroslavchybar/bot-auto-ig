...
from python.core.storage.selector_cache import (
    load_cache, save_cache, record_success, get_preferred_strategy
)
from python.core.selectors import SemanticSelector
...
def mock_cache_file(tmp_path):
    ...
    with patch("python.core.storage.selector_cache.CACHE_FILE", cache_file):
        yield cache_file
...
def test_find_text_fallback(mock_cache_file, mock_page, mock_locator):
    ...
    fallback_candidate.is_visible.return_value = True
    fallback_candidate.is_enabled.return_value = True
    fallback_locator.nth.return_value = fallback_candidate
    mock_page.locator.return_value.filter.return_value = fallback_locator
