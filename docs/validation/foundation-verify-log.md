...
| `python -m pytest python/tests -q` | 0 | 221 passed, 1 xfailed, 0 failures |
...
- Route-level error boundaries report to Sentry via `RouteErrorView.tsx`

## Pre-existing Known Issues (Not Regressions)

- `test_multi_account_cooperative_stop_emits_success_before_idle_sync`: marked `@pytest.mark.xfail` — expects 'success' but gets 'cancelled' (pre-existing behavior mismatch)
